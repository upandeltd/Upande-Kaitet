...
def get_sensor_chart_data(sensor_name=None, date_from=None, date_to=None, timespan=None, time_interval=None):
	...
	# Sensors for which we SUM values instead of AVG
	sum_sensors = ["energy", "precipitation", "level"]

	# Map sensor name to its unit
	sensor_unit_map = {
		"kaitet_greenhouse1": "µS/cm",
		"temperature_sensor1": "°C",
		"ph_sensor1": "pH",
		"flow_sensor1": "L/min",
		"level_sensor1": "m",
		"energy_meter1": "kWh",
	...
	unit = sensor_unit_map.get(sensor_name, "")
	...
	if date_from and not date_to:
		try:
			...
			end_dt = round_up_to_next_hour(start_dt.replace(hour=23, minute=59, second=59))
	...
	if not date_to:
		end_dt = round_up_to_next_hour(now)
		date_to = end_dt.strftime("%Y-%m-%d %H:%M:%S")
	...
	# Decide aggregation function based on sensor
	agg_func = "SUM" if sensor_name in sum_sensors else "AVG"

	# Quarterly special case
	...
	# General aggregation for other intervals
	...
	# Optional: De-clutter hourly labels
	if time_interval == "hourly":
		if timespan == "last_week":
			formatted_labels = [lbl if i % 4 == 0 else "" for i, lbl in enumerate(formatted_labels)]
		elif timespan == "last_month":
			formatted_labels = [lbl if i % 12 == 0 else "" for i, lbl in enumerate(formatted_labels)]
